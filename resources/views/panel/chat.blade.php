...
<div id="chat-list-panel" class="w-full md:w-1/3 h-full flex-shrink-0 p-5 md:pe-0">
    <div class="flex flex-col h-full bg-gray-800 rounded-xl border border-gray-700">
        ...
        <div class="p-4 border-b border-gray-700">
            <input type="text" placeholder="Buscar un chat" class="w-full px-4 py-2 text-white text-sm bg-gray-700 rounded-lg focus:outline-none placeholder-slate-400 focus:ring-2 focus:ring-indigo-500">
        </div>
...
<template id="chat-window-template">
    <div class="flex flex-col h-full bg-gray-800 relative rounded-xl border border-gray-700">
        <div class="flex items-center px-3 py-2 bg-gray border-b border-gray-700 shadow-sm">
            ...
            <div class="flex items-center justify-between px-1 py-2 w-[90%] md:w-full">
                <!-- Contenedor del perfil de chat. Se expandirá y contraerá. -->
                <div class="chat-profile-btn flex items-center rounded-xl hover:bg-gray-600 transition-colors duration-200 cursor-pointer min-w-0 mr-4">
                    <!-- El avatar no se encogerá -->
                    <img class="chat-avatar w-8 h-8 rounded-full mr-2 flex-shrink-0"
                        onerror="this.onerror=null;this.src='https://placehold.co/100x100/364153/FFFFFF?text=U';">
                    <!-- El nombre se truncará si es demasiado largo -->
                    <h3 class="chat-name font-semibold text-white truncate pe-3"></h3>
                </div>

                <!-- Switch -->
                <div class="flex-shrink-0">
                    <label class="relative inline-flex items-center cursor-pointer" data-modal-open="changeStatusModal">
        ...
        <div class="relative flex-1 p-4 overflow-y-auto ">
            <div class="absolute inset-0"
                style="background-image: url('{{ asset('img/bg_tiled.png') }}'); background-repeat: repeat; opacity:0.1; pointer-events:none; background-size:220px;">
            </div>
            <div id="messages-container" class="space-y-3"></div>
        </div>
        <div class="flex items-center p-4 border-t border-gray-700">
            <input type="text" id="message-input" autocomplete="off"
                class="message-input flex-1 px-4 py-2 text-sm bg-gray-700 rounded-full focus:outline-none placeholder-slate-400 text-white focus:ring-2 focus:ring-indigo-500"
                placeholder="Escribe un mensaje">
            <button id="send-button"
                class="ml-2 p-2 btn-custom rounded-full transition-transform transform active:scale-95 cursor-pointer">
        ...
        <div class="chat-profile-box absolute top-0 inset-x-0 z-10">
            <div class="w-full h-full relative">
                <button
                    class="chat-profile-close-btn absolute top-2 right-2 mr-1 text-xl text-white hover:text-gray-500">✕</button>
                <div
                    class="w-full h-full rounded-lg shadow-xl p-1 h-10 flex border border-gray-600 rounded-xl bg-slate-800">
                    <!-- Panel de izquierda -->
                    <div class="w-1/3 pr-1">
                        <ul class="space-y-2">
                            <li>
                                <button class="chat-profile-tab-btn tab-btn flex w-full text-xs text-left py-2 px-2 rounded-lg"
                                    data-tab="profile">
                                    <div class="mr-2">
                            ...
                            <li>
                                <button
                                    class="chat-user-tab-btn tab-btn flex w-full text-xs text-left py-2 px-2 rounded-lg hover:bg-gray-600 hover:text-slate-200"
                                    data-tab="user">
                                    <div class="mr-2">
...
<div id="changeStatusModal" data-on-open="updateStatusModalText"
    ...
    <div class="card-custom rounded-xl w-96 p-6 relative shadow-2xl text-center">
        <button id="closeChangeStatusModal" data-modal-close
            class="absolute top-2 right-2 text-white hover:text-gray-400 transition-colors cursor-pointer">
        ...
        <p id="changeStatusModalText" class="text-gray-300 mb-6"></p>
        <div class="flex justify-center space-x-4">
            <button id="cancelChangeStatusBtn" data-modal-close
                class="px-5 py-2 bg-gray-500 text-white font-medium rounded-lg hover:bg-gray-600 transition-colors w-30 cursor-pointer">
                Cancelar
            </button>
            <button id="confirmChangeStatusBtn"
                class="px-5 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 transition-colors w-30 cursor-pointer">
                Confirmar
            </button>
        </div>
    </div>
